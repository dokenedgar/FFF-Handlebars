...
describe('ADMIN API TESTS', function () {
  ...
  it('login in an admin', function () {
    return _chai2.default.request(_index2.default).post('/api/v1/admin').send({
      username: 'sergio',
      password: 'ramos'
    }).then(function (res) {
      (0, _chai.expect)(res).to.have.status(200);
  ...
  it('admin - get a users specific order', function () {
    return _chai2.default.request(_index2.default).get('/admin/orders/orderid').then(function (res) {
      (0, _chai.expect)(res).to.have.status(200);
    });
  });

  it('api for getting users orders', function () {
    return _chai2.default.request(_index2.default).get('/api/v1/admin/orders/orderID').set('authorization', 'Bearer ' + adminJwtToken).then(function (res) {
      (0, _chai.expect)(res).to.have.status(200);
      (0, _chai.expect)(res.body).to.be.an('object');
    });
  });
  ...
  it('admin - api endpoint for adding food', function () {
    return _chai2.default.request(_index2.default).post('/api/v1/admin/food').set('authorization', 'Bearer ' + adminJwtToken).send({
      foodName: 'Rice',
      foodPrice: '350',
  ...
  it('api updating user orders status', function () {
    return _chai2.default.request(_index2.default).put('/api/v1/admin/food').set('authorization', 'Bearer ' + adminJwtToken).send({
      foodName: 'Rice',
      foodPrice: '1050',
      foodDesc: ' a way to advertise and describe the food'
    }).then(function (res) {
  ...
  it('admin - api for deleting a food from the menu', function () {
    return _chai2.default.request(_index2.default).delete('/api/v1/admin/food').set('authorization', 'Bearer ' + adminJwtToken).send({
      foodName: 'Rice'
    }).then(function (res) {
      (0, _chai.expect)(res).to.have.status(200);
      (0, _chai.expect)(res.body).to.be.an('object');
